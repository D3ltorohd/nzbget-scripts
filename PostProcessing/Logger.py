...
# Pfad angabe !!!!WICHTIG!!!!.
# Hier bitte den Pfad angeben wo die .txt Datei von Logger.py gesichert werden kann!!.
#PATH_FILE=
...
if not 'NZBOP_SCRIPTDIR' in os.environ:
        print("[INFO] *** NZBGet post-processing script ***")
        print("[INFO] This script is supposed to be called from nzbget (11.0 or later).")
        sys.exit(POSTPROCESS_ERROR)
 
if not os.path.exists(os.environ['NZBPO_PATH_FILE']):
...
if len(log) > 0:
        log_datei = ("%s/_postprocesslog.txt") % (os.environ['NZBPO_PATH_FILE'])
        f = open(log_datei, 'wb')
        for entry in log:
                f.write((u'%s\t%s\t%s\n' % (entry['Kind'], datetime.datetime.fromtimestamp(int(entry['Time'])), entry['Text'])).encode('utf8'))
                print "[INFO] Die Logger.py Datei: [%s] wurde erfolgreich erstellt!!...." % (log_datei)
        f.close()
